...
class LegalAiController extends Controller
{
    ...
    public function ask(Request $request)
    {
        ...
        $request->validate([
            ...
            'mode'              => 'nullable|string|in:simplified,professional',
        ]);

        $question = $request->question;
        $uuid     = $request->conversation_uuid;
        $mode     = $request->input('mode', 'simplified');
        ...
        if ($isDirectArticleRequest) {
            $structureInstruction = $articleOnlyRule;
        } elseif ($mode === 'simplified') {
            $structureInstruction = "
🎯 وضع الإجابة: إجابة مبسطة للمستفيدين والأفراد (غير المحامين):
- المستفيد شخص عادي يبحث عن فهم حقوقه وواجباته وإجراءاته بوضوح ويسر.
- صغ الإجابة بلغة عربية سلسة ومباشرة خالية من الحشو والغموض والتعقيد اللفظي أو القضائي.
- اشرح بأسلوب عملي ما له وما عليه وما هي الخطوات العملية الملموسة التي ينبغي اتخاذها لحماية حقه.
- يجب التوثيق التام والصارم بأرقام المواد والأنظمة السعودية المسترجعة في السياق أدناه.

🏛️ الهيكلية البصرية الإلزامية للمخرجات (Structured Markdown):
صغ الإجابة وفق القالب التالي:

### 💡 خلاصة الموقف وحقك النظامي
[شرح مبسط ومباشر لموقفك النظامي وما تستحقه أو ما يفرضه عليك النظام]

### 📋 الخطوات العملية والإجراءات الموصى بها
1. [الخطوة الأولى: مثال اللجوء للتسوية الودية، منصة قوى، منصة ناجز، إخطار الطرف الآخر...]
2. [الخطوة الثانية: الإجراء التالي...]
3. [الخطوة الثالثة...]

### 📄 المستندات والأوراق المطلوبة لدعم موقفك
- [العقد الموثق، إشعارات المراسلات، كشوف الحساب، الفواتير، إلخ]

---
### 📚 المواد والأنظمة السعودية التي تضمن حقك:
- **[اسم النظام السعودي، مثال: نظام العمل أو نظام المعاملات المدنية]:** المادة ([رقم المادة]) - [شرح مختصر وميسر لحكم المادة وكيف تحمي حقك].
- **[اللائحة التنفيذية (إن وجدت)]:** المادة ([رقم المادة]).
";
        } else {
            $structureInstruction = "
🎯 وضع الإجابة: إجابة احترافية متخصصة (للمحامين والباحثين والمستشارين):
- قدّم تحليلاً قضائياً متعمقاً، والتكييف الفقهي والنظامي للوقائع، مع استعراض السوابق القضائية والأسباب والمدد الإجرائية.

🏛️ الهيكلية البصرية الإلزامية للمخرجات (Structured Markdown):
صغ الإجابة وفق القالب المهيكل التالي:

### 📌 الرأي والمشورة القانونية
[إجابة حاسمة ومباشرة وواضحة لسؤال المستفيد]

### ⚖️ التحليل والتطبيق النظامي والتكييف القضائي
[التحليل الموضوعي والإجرائي، والتكييف النظامي، وربط الوقائع بالنصوص النظامية، وتطبيق المبادئ القضائية إن وجدت، مع ذكر المدد الإجرائية إن انطبقت]
