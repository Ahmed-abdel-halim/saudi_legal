...
<body class="bg-gray-50 overflow-y-auto custom-scrollbar flex flex-col min-h-screen">
    ...
    <header class="flex items-center justify-between px-8 py-5 bg-white border-b border-gray-100 sticky top-0 z-50">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-black text-gray-800 tracking-wide">Radiif <span class="text-blue-600">Legal AI</span></h1>
        </div>
        <div class="flex items-center gap-4 text-sm font-bold">
            <span class="text-gray-500">المنجز اليوم: <span class="text-emerald-600">{{ $stats['completed_today'] }}</span></span>
    ...
    <main class="flex-1 w-full max-w-4xl mx-auto py-10 px-4">
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden relative">
            ...
            <div class="p-8 md:p-12 space-y-8">
                ...
                <div class="space-y-4 pt-4">
                    <div class="flex justify-between items-center px-2">
                        <span class="text-xs text-gray-400 font-bold flex items-center gap-2">
                            <i class="fa-solid fa-circle-info text-gray-300"></i> نظام الإثبات: انقر على أي كلمة لتحديدها كخطأ
                        </span>
                ...
                <!-- Primary Source -->
                <div class="text-center text-sm font-bold text-gray-500 bg-gray-50/50 py-3 rounded-xl border border-dashed border-gray-200">
                ...
                <div class="border border-gray-100 rounded-2xl p-5">
                    ...
                    <div class="flex flex-wrap justify-center gap-3">
                        ...
                        <label class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                ...
                <div id="action-buttons" class="grid grid-cols-2 gap-4 pt-4">
                    <button onclick="toggleCorrection(true)" class="group flex flex-col items-center justify-center p-6 bg-rose-50 border border-rose-100 rounded-3xl hover:bg-rose-100 transition duration-300">
                        <div class="w-12 h-12 bg-rose-500 text-white rounded-full flex items-center justify-center text-xl mb-3 shadow-md shadow-rose-200 group-hover:-translate-y-1 transition transform">
                    ...
                    <button onclick="submitTask(true)" class="group flex flex-col items-center justify-center p-6 bg-emerald-50 border border-emerald-100 rounded-3xl hover:bg-emerald-100 transition duration-300">
                        <div class="w-12 h-12 bg-emerald-500 text-white rounded-full flex items-center justify-center text-xl mb-3 shadow-md shadow-emerald-200 group-hover:-translate-y-1 transition transform">
                ...
                <div id="correction-area" class="hidden space-y-4 pt-4 animate-in fade-in slide-in-from-top-4 duration-300">
                    <div class="p-1 bg-rose-50 rounded-2xl border border-rose-100">
                        <textarea id="correct_answer" class="w-full h-40 p-5 bg-transparent border-none focus:ring-0 outline-none text-lg font-bold text-gray-800 resize-none" placeholder="اكتب التعديل الصحيح هنا..."></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button onclick="toggleCorrection(false)" class="px-6 py-4 bg-gray-100 text-gray-600 font-black rounded-xl hover:bg-gray-200 transition">إلغاء</button>
                        <button onclick="submitTask(false)" id="btn-submit-edit" class="flex-1 flex justify-center items-center gap-2 px-6 py-4 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-200">
                    ...
                    <div class="space-y-3 text-right">
                        ...
                        <p class="text-sm text-gray-600 leading-relaxed font-medium max-h-64 overflow-y-auto custom-scrollbar pl-2 whitespace-pre-wrap">
                            @php
                                ...
                                $sysName = $task->law_system_name;

                                // If the article is not defined, fetch a real one from the database dynamically
                                if (strpos($articleText, 'يرجى البحث') !== false || $articleNum == 'غير محدد') {
                                    $searchKeyword = isset($keywords[1]) ? $keywords[1] : (isset($keywords[0]) ? $keywords[0] : 'نظام');
                                    $fallbackArticle = \App\Models\LegalArticle::where('content', 'LIKE', '%' . $searchKeyword . '%')
                                        ->orWhere('legislation_title', 'LIKE', '%' . $sysName . '%')
                                        ->inRandomOrder()
                                        ->first();
                                    
                                    if ($fallbackArticle) {
                                        $articleText = strip_tags($fallbackArticle->content);
                                        $articleNum = str_replace('المادة ', '', $fallbackArticle->article_title);
                                        $sysName = $fallbackArticle->legislation_title;
                                    }
                                }
                            @endphp
            ...
            <div class="bg-gray-50/50 px-8 py-4 border-t border-gray-100 flex justify-between items-center">
                <button onclick="skipTask()" class="text-gray-400 hover:text-gray-800 font-bold text-sm flex items-center gap-2 transition">
                    تخطي <i class="fa-solid fa-forward-step rtl:rotate-180"></i>
                </button>
                <button onclick="previousTask()" class="text-gray-400 hover:text-gray-800 font-bold text-sm flex items-center gap-2 transition">
    ...
    <main class="flex-1 flex items-center justify-center">
        <div class="text-center p-12 bg-white rounded-[3rem] shadow-sm border border-gray-100 max-w-md w-full">
            ...
            <p class="text-gray-500 text-sm font-medium mb-8 leading-relaxed">لقد قمت بتنقيح كل البيانات المتاحة في القائمة. يمكنك العودة لاحقاً للمزيد.</p>
            <a href="{{ route('dashboard.expert') }}" class="inline-flex items-center gap-2 px-8 py-4 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl font-bold transition-all shadow-lg shadow-emerald-200">
