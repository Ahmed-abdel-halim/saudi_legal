...
@section('title', 'إنشاء باقة جديدة')
...
<div class="max-w-3xl mx-auto space-y-6" dir="rtl">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-extrabold text-white">إنشاء باقة جديدة</h1>
            <p class="text-slate-400 text-sm">أضف باقة اشتراك جديدة للمساعد الذكي القانوني</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.ai_packages.store') }}" class="space-y-5">
        @csrf
        ...
        <div class="bg-sidebar rounded-2xl border border-slate-700/50 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="مثال: باقة المحترف القانوني"
                ...
                <div>
                    <label class="block text-slate-300 text-sm font-semibold mb-1.5">نص الشارة (اختياري)</label>
                    <input type="text" name="badge_text" value="{{ old('badge_text') }}" placeholder="مثال: الأكثر طلباً 🚀"
                           class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="وصف مختصر يظهر للمستخدمين..."
                          class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition resize-none">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-sidebar rounded-2xl border border-slate-700/50 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-slate-300 text-sm font-semibold mb-1.5">فترة الفوترة <span class="text-red-400">*</span></label>
                    <select name="billing_period"
                            class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500 transition">
                        <option value="monthly" {{ old('billing_period') === 'monthly' ? 'selected' : '' }}>شهري</option>
                        <option value="yearly" {{ old('billing_period') === 'yearly' ? 'selected' : '' }}>سنوي</option>
                        <option value="lifetime" {{ old('billing_period') === 'lifetime' ? 'selected' : '' }}>مدى الحياة</option>
                    </select>
                </div>
                <div>
                    <label class="block text-slate-300 text-sm font-semibold mb-1.5">عدد الاستعلامات <span class="text-red-400">*</span></label>
                    <input type="number" name="query_limit" value="{{ old('query_limit', 100) }}" min="-1"
                           placeholder="-1 للامحدود"
                           class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500 transition">
                    <p class="text-slate-500 text-xs mt-1">أدخل -1 للاستخدام اللامحدود</p>
                </div>
            </div>
        ...
        <div class="bg-sidebar rounded-2xl border border-slate-700/50 p-6 space-y-5">
            ...
            <div id="features-container" class="space-y-2.5">
                <div class="flex gap-2">
                    <input type="text" name="features[]" placeholder="مثال: استعلامات غير محدودة طوال الشهر"
        ...
        <div class="bg-sidebar rounded-2xl border border-slate-700/50 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-slate-300 text-sm font-semibold mb-1.5">Stripe Price ID (اختياري)</label>
                    <input type="text" name="stripe_price_id" value="{{ old('stripe_price_id') }}" placeholder="price_xxxxx"
                           class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition font-mono text-xs">
                </div>
                <div>
                    <label class="block text-slate-300 text-sm font-semibold mb-1.5">لون الكارت</label>
                    <select name="color_scheme"
                            class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500 transition">
                        <option value="emerald" {{ old('color_scheme') === 'emerald' ? 'selected' : '' }}>🟢 أخضر (Emerald)</option>
                        <option value="indigo" {{ old('color_scheme') === 'indigo' ? 'selected' : '' }}>🔵 أزرق (Indigo)</option>
                        <option value="gold" {{ old('color_scheme') === 'gold' ? 'selected' : '' }}>🟡 ذهبي (Gold)</option>
                        <option value="slate" {{ old('color_scheme') === 'slate' ? 'selected' : '' }}>⚫ رمادي (Slate)</option>
                    </select>
                </div>
                <div>
        ...
        <div class="flex items-center justify-end gap-3 pb-4">
            <a href="{{ route('admin.ai_packages.index') }}"
               class="px-5 py-2.5 rounded-xl border border-slate-600 text-slate-300 hover:bg-slate-700 font-semibold text-sm transition">
                إلغاء
            </a>
            <button type="submit"
