...
<head>
    ...
    <title>{{ __('services.CONTACT_COMPANY_TITLE') }} | {{ $platformName }}</title>
        ...
        <div class="container mx-auto px-4 max-w-2xl relative z-10">
            ...
            <div class="bg-white rounded-3xl shadow-2xl shadow-slate-200/50 p-8 md:p-12 border border-slate-100">
                ...
                <div class="text-center mb-10">
                    ...
                    <h1 class="text-3xl font-extrabold text-slate-900 mb-3">{{ __('services.CONTACT_COMPANY_TITLE') }}</h1>
                ...
                <!-- Contact Form -->
                <form action="{{ route('services.contact.send', $service->service_id) }}" method="POST" class="space-y-8">
                    @csrf
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2.5">{{ __('services.FORM_NAME') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2.5">{{ __('services.FORM_EMAIL') }}</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 rtl:right-0 rtl:left-auto rtl:pr-3 rtl:pl-0">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"/>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2.5">{{ __('services.FORM_MESSAGE') }}</label>
                        <textarea name="message" rows="5" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition bg-slate-50/50 focus:bg-white resize-none"></textarea>
                    </div>
                    ...
                    <div class="pt-4 flex flex-col items-center gap-4">
                        <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl transition shadow-lg shadow-indigo-200 transform hover:-translate-y-1 flex items-center justify-center gap-2">
                            <span>{{ __('services.BTN_SEND_MESSAGE') }}</span>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
