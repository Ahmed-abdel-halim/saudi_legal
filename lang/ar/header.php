<?php

return [
    // Platform
    'PLATFORM_NAME' => 'رديف',
    
    // Navigation
    'NAV_HOW_IT_WORKS' => 'ألية العمل',
    'NAV_SERVICES' => 'تصفح الخدمات',
    'NAV_REQUESTS' => 'تصفح الطلبات',
    'NAV_SUPPLIERS' => 'الشركات الموردة',
    'NAV_PRODUCTS' => 'منتجات رديف',
    'PRODUCT_LEGAL' => 'المساعد القانوني الذكي',
    'PRODUCT_LEGAL_DESC' => 'استشارات وسوابق قضائية وأنظمة سعودية',
    'PRODUCT_FATWA' => 'المفتي الإسلامي الذكي',
    'PRODUCT_FATWA_DESC' => 'فتاوى موثقة ومؤصلة بأدلة الكتاب والسنة الصحيحة',
    'BADGE_BETA' => 'جديد',
    'NAV_AI_ASSISTANT' => 'المساعد القانوني',
    'NAV_PACKAGES' => 'الباقات والأسعار',
    'NAV_MESSAGES' => 'الرسائل',
    
    // Buttons
    'BTN_DASHBOARD' => 'لوحة التحكم',
    'BTN_LOGIN' => 'تسجيل الدخول',
    'BTN_LOGOUT' => 'تسجيل الخروج',
    'BTN_START_NOW' => 'ابدأ الآن',
    
    // Language Switcher
    'LANG_ARABIC' => 'العربية',
    'LANG_ENGLISH' => 'English',
    
    // Accessibility
    'ARIA_TOGGLE_MENU' => 'فتح/إغلاق القائمة',
    
    // Notifications
    'NOTIFICATIONS' => 'الإشعارات',
    'MARK_ALL_READ' => 'تعليم الكل كمقروء',
    'NO_NOTIFICATIONS' => 'لا توجد إشعارات بعد',
    'VIEW_ALL_NOTIFICATIONS' => 'عرض جميع الإشعارات',
    
    // Messages
    'LOGOUT_SUCCESS' => 'تم تسجيل الخروج بنجاح',
];
